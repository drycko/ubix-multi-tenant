  ...
  public function handle()
  {
    ...
    foreach ($subscriptions as $subscription) {
      try {
        ...
        $nextStartDate = $subscription->end_date->copy()->addDay();
        $nextEndDate = $this->calculateNextEndDate($subscription, $nextStartDate);
        
        // Check if invoice for this exact period already exists
        $existingInvoice = SubscriptionInvoice::where('subscription_id', $subscription->id)
        ->where('status', '!=', 'cancelled')
        ->where(function ($query) use ($nextStartDate, $nextEndDate) {
          // Check if invoice notes contain this period (more reliable)
          $query->where('notes', 'like', "%{$nextStartDate->format('M d, Y')}%")
          ->where('notes', 'like', "%{$nextEndDate->format('M d, Y')}%");
        })
        ->orWhere(function ($query) use ($subscription, $targetDate) {
          // Alternative: Check by due date matching subscription end date
        ...
        if ($existingInvoice) {
          $this->warn("⊘ Invoice already exists for {$subscription->tenant->name} (Subscription #{$subscription->id}) for period {$nextStartDate->format('M d, Y')} - {$nextEndDate->format('M d, Y')}");
          continue;
        }
