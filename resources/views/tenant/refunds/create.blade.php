    ...
    <div class="row">
      <div class="col-sm-6">
        <h3 class="mb-0 text-muted">
          <i class="fas fa-undo-alt"></i> New Refund
        </h3>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-end">
          ...
          <li class="breadcrumb-item active" aria-current="page">New Refund</li>
        </ol>
      </div>
    </div>
  </div>
    ...
    <div class="row">
      <div class="col-xl-8 col-lg-7">
        <div class="card card-info card-outline">
          ...
          <form action="{{ route('tenant.refunds.store') }}" method="POST">
            @csrf
            <div class="card-body">
              <div class="mb-3">
                ...
                <select name="payment_id" id="payment_id" class="form-select @error('payment_id') is-invalid @enderror" required>
                  ...
                  @foreach($payments as $payment)
                    <option value="{{ $payment->id }}" {{ old('payment_id') == $payment->id ? 'selected' : '' }} data-invid="{{ $payment->booking_invoice_id }}" data-amount="{{ $payment->refundable_amount }}">#{{ $payment->id }} - {{ $currency }} {{ number_format($payment->amount, 2) }}</option>
                  @endforeach
                </select>
                @error('payment_id')
              ...
              <div class="mb-3">
                ...
                <select name="invoice_id" id="invoice_id" class="form-select @error('invoice_id') is-invalid @enderror" readonly disabled>
                  ...
                  @foreach($invoices as $invoice)
                    <option value="{{ $invoice->id }}" {{ old('invoice_id') == $invoice->id ? 'selected' : '' }}>#{{ $invoice->invoice_number }} - {{ $currency }} {{ number_format($invoice->amount, 2) }}</option>
                  @endforeach
                </select>
                @error('invoice_id')
                  <span class="invalid-feedback">{{ $message }}</span>
            ...
            <div class="card-footer text-end">
              <button type="submit" class="btn btn-info">
                <i class="fas fa-save me-1"></i>Submit Refund
              </button>
