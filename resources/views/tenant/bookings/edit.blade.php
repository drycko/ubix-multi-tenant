    ...
    <div class="row">
      <div class="col-sm-6">
        <h3 class="mb-0">
          ...
          <small class="text-muted">Edit Booking</small>
        </h3>
      </div>
      <div class="col-sm-6">
...
<div class="app-content">
  <div class="container-fluid">
    ...
    <form action="{{ route('tenant.bookings.update', $booking->id) }}" method="POST" id="bookingForm">
      @csrf
      @method('PUT')
              ...
              <div class="row mb-3">
                ...
                <div class="col-md-4">
                  ...
                  <input type="date" 
                         ...
                         id="departure_date" 
                         name="departure_date" 
                         value="{{ old('departure_date', $departureDate) }}" 
                         required>
                ...
                <div class="col-md-4">
                  ...
                  <select class="form-select @error('is_shared') is-invalid @enderror" id="is_shared" name="is_shared">
                    <option value="0" {{ old('is_shared', $booking->is_shared) == 0 ? 'selected' : '' }}>Private Room</option>
                    <option value="1" {{ old('is_shared', $booking->is_shared) == 1 ? 'selected' : '' }}>Shared Room</option>
                  </select>
                  @error('is_shared')
                  <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
              ...
              <div class="row mb-3">
                <div class="col-md-4">
                  ...
                  <select class="form-select @error('room_id') is-invalid @enderror" id="room_id" name="room_id" required>
                    ...
                    <option value="{{ $room->id }}" 
                            ...
                            {{ old('room_id', $booking->selected_room) == $room->id ? 'selected' : '' }}>
                ...
                <div class="col-md-2">
                  ...
                  <input type="number" 
                         ...
                         id="adults" 
                         name="adults" 
                         value="{{ old('adults', $booking->count_adults) }}" 
                         min="1" 
                         required>
                  @error('adults')
                  <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-2">
                  ...
                  <input type="number" 
                         ...
                         id="children" 
                         name="children" 
                         value="{{ old('children', $booking->count_children) }}" 
                         min="0">
                  @error('children')
                  <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                ...
                <div class="col-md-4">
                  ...
                  <input type="number" 
                         ...
                         id="daily_rate" 
                         name="daily_rate" 
                         value="{{ old('daily_rate', $booking->daily_rate) }}" 
                         required>
                  @error('daily_rate')
                  <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
              ...
              <div class="row mb-3">
                ...
                <div class="col-md-6">
                  ...
                  <select class="form-select @error('source') is-invalid @enderror" name="source" id="source" required>
                    ...
                    <option value="{{ $source }}" {{ old('source', $booking->source) == $source ? 'selected' : '' }}>
            ...
            <div class="card-body">
              ...
              <div class="mb-3">
                ...
                <select class="form-select @error('guest_id') is-invalid @enderror" id="guest_id" name="guest_id" required>
                  ...
                  <option value="{{ $guest->id }}" {{ old('guest_id', $booking->primary_guest_id) == $guest->id ? 'selected' : '' }}>
              ...
              <div class="mb-3">
                ...
                <textarea class="form-control @error('special_requests') is-invalid @enderror" 
                          ...
                          rows="3" 
                          placeholder="Any special requirements or requests...">{{ old('special_requests', $booking->g1_special_requests) }}</textarea>
              ...
              @php 
              $secondaryGuest = $booking->guest2_id ? $booking->guests->where('id', $booking->guest2_id)->first() : null;
              $g2_special_requests = '';
              if ($secondaryGuest) {
                $g2_special_requests = $secondaryGuest->special_requests ?? '';
              }
              @endphp
              <div id="secondary_guest_section" @if(!$booking->guest2_id) style="display: none;" @endif>
                <hr>
                <div class="mb-3">
                  ...
                  <select class="form-select @error('guest2_id') is-invalid @enderror" id="guest2_id" name="guest2_id">
                    ...
                    @foreach($guests as $guest)
                    <option value="{{ $guest->id }}" {{ old('guest2_id', $secondaryGuest->id ?? null) == $guest->id ? 'selected' : '' }}>
                      {{ $guest->first_name }} {{ $guest->last_name }}
                      @if($guest->email) - {{ $guest->email }} @endif
                    </option>
                    @endforeach
                ...
                <div class="mb-3">
                  ...
                  <textarea class="form-control @error('g2_special_requests') is-invalid @enderror" 
                            ...
                            rows="2" 
                            placeholder="Special requests for secondary guest...">{{ old('g2_special_requests', $g2_special_requests) }}</textarea>
...
<script>
  ...
  document.getElementById('departure_date').addEventListener('change', function() {
    ...
    .then(data => {
      ...
      data.rooms.forEach(room => {
        ...
        // Restore selection if it was the previously selected room
  ...
  document.getElementById('bookingForm').addEventListener('submit', function() {
    ...
    submitBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Updating Booking...';
  });
</script>
@endsection