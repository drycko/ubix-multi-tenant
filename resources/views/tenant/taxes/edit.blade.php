        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">
                        <i class="fas fa-edit"></i> Edit Tax: {{ $tax->name }}
                    </h3>
                </div>

                <form action="{{ route('tenant.taxes.update', $tax) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                                    ...
                                    <select name="property_id" id="property_id" class="form-select @error('property_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($properties as $property)
                                            <option value="{{ $property->id }}" 
                                                    {{ (old('property_id', $tax->property_id) == $property->id) ? 'selected' : '' }}>
                                                {{ $property->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <textarea name="description" id="description" 
                                              ...
                                              rows="3" placeholder="Optional description for this tax">{{ old('description', $tax->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" name="is_inclusive" id="is_inclusive" 
                                               value="1" {{ old('is_inclusive', $tax->is_inclusive) ? 'checked' : '' }}>
                    ...
                    <div class="card-footer">
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="d-flex gap-2">
                                <a href="{{ route('tenant.taxes.show', $tax) }}" class="btn btn-outline-primary">
